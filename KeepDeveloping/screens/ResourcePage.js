...
import { StyleSheet, View, ScrollView, StatusBar, Text, Image } from "react-native";
        ...
        content.map((prop, key) => {
            if (key != 0) {
                switch (prop.styles.toLowerCase()) {
                    case "texto":
                        components = [...components, <View><Text style={styles.text}>{prop.value}</Text></View>]
                        break;
                    case "imagem":
                        components = [...components, <View><Image style={styles.image} resizeMode="contain" source={prop.value == "Agile" ? agile : prop.value == "Agile_planning" ? agilePlanning : scrum} /></View>]
                        break;
                    case "video":
